...
# Private Lib
from Preprocess import GraphStruct2XandA
...
def read_data(
    ...
    """
    root_path = "../datasets/"
    indicator_path = root_path + str(name) + "/" + str(name) + "_graph_indicator.txt"
    edges_path = root_path + str(name) + "/" + str(name) + "_A.txt"
    node_labels_path = root_path + str(name) + "/" + str(name) + "_node_labels.txt"
    node_attributes_path = root_path + str(name) + "/" + str(name) + "_node_attributes.txt"
    edge_labels_path = root_path + str(name) + "/" + str(name) + "_edge_labels.txt"
    edge_attributes_path = \
        root_path + str(name) + "/" + str(name) + "_edge_attributes.txt"
    graph_classes_path = \
        root_path + str(name) + "/" + str(name) + "_graph_labels.txt"
...
def Preprocess_GandY(X, Y):
    NodeFeatures = list()
    AdjMat = list()

    OriginData = X
    for origindata in OriginData:
        X_temp, A_temp = GraphStruct2XandA(origindata[0], origindata[1])
        NodeFeatures.append(X_temp)
        AdjMat.append(A_temp)

    # 二分类 标签为 -1 1时+
    y = np.array(Y)
    # y_train = np.array(minibatch_Y) + np.ones(np.array(minibatch_Y).shape)
    NodeFeatures = np.array(NodeFeatures)
    AdjMat = np.array(AdjMat)
   
    return NodeFeatures, AdjMat, y
