...
from GraphClassification.Model.GraphSAGE_GraphClassifier import GraphClassifier
...
class GraphFuzzySystem(nn.Module):
    def __init__(self, _X_train: list, _Y_train: list, _X_val: list, _Y_val: list, _X_test: list, _Y_test: list,
        ...
        self.saved_path = './TempModel/{}/{}_GFS-GraphSAGE_{}_{}.pkl'.format(self.DatasetName, self.DatasetName, self.DEVICE,
                                                                       TimeStr)
    ...
    def fit(self):
        ...
        for epoch in range(self.EPOCHS):
            ...
            for minibatch in minibatches:
                ...
                train_rmse = mean_squared_error(y_train.cpu().detach().numpy(), logits.max(1)[1].cpu().detach().numpy(),
                                                squared=False)
                ...
                loss_val = self.criterion(logits_val.to(self.DEVICE), torch.DoubleTensor(y_val).to(self.DEVICE))
                ...
                logger.info(
                    'Epoch:{:05d}/{:05d}--Batch:{:03d}/{:03d}|Train Loss:{:.6f},Train RMSE: {:.4}|Val Loss:{:.6f},Val RMSE:{:.4}'.format(
                        epoch + 1, self.EPOCHS, Count_NumBatch, num_batch, loss.item(), train_rmse, loss_val.item(),
                        val_rmse))
        ...
        with torch.no_grad():
            ...
            for k in range(self.num_rules):
                ...
                if k == 0:
                    ...
                    logits_test += temp_test
            # test_loss = self.criterion(logits_test.to(self.DEVICE), torch.LongTensor(y_test).to(self.DEVICE))
            # test_acc = torch.eq(logits_test.max(1)[1], torch.tensor(y_test).to(self.DEVICE)).float().mean()
            test_loss = torch.sqrt(
                self.criterion(logits_test.to(self.DEVICE), torch.DoubleTensor(y_test).to(self.DEVICE)))
            # test_acc = torch.eq(logits_test.max(1)[1], torch.tensor(y_test).to(self.DEVICE)).float().mean()
