...
from GraphClassification.Model.GCN_GraphClassifier import GraphClassifier
...
class GraphFuzzySystem(nn.Module):
    def __init__(self, _X_train: list, _Y_train: list, _X_val: list, _Y_val: list, _X_test: list, _Y_test: list,
        ...
        self.saved_path = './TempModel/{}/{}_GFS-GCN-Regression_{}_{}.pkl'.format(self.DatasetName, self.DatasetName, self.DEVICE,
                                                                       TimeStr)
    ...
    def fit(self):
        ...
        for epoch in range(self.EPOCHS):
            ...
            for minibatch in minibatches:
                ...
                val_rmse = mean_squared_error(y_val, logits_val.max(1)[1].cpu().detach().numpy())
    ...
    def predict(self):
        ...
        with torch.no_grad():
            ...
            test_loss = self.criterion(logits_test.to(self.DEVICE), torch.DoubleTensor(y_test).to(self.DEVICE))
            # test_acc = torch.eq(logits_test.max(1)[1], torch.tensor(y_test).to(self.DEVICE)).float().mean()
            test_rmse = mean_squared_error(y_test, logits_test.max(1)[1].cpu().detach().numpy())
